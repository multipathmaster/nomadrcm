...
generate_new_token(){
curl -s ${RSERV}/api/v1/login -d ${AUTH} | jq -r '' | egrep '(authToken|userId)' | sed s/\ //g | awk -F "\"" '{ print $4 }' > rocketcauth.$$
}
...
curl -s -H "${TT}: ${AUTHT}"      -H "${XUI}: ${USRHSH}"      -H "${CS}:${CONT}"      ${RSERV}/api/v1/chat.postMessage      -d '{ "channel": "#general", "text": "Zombies!" }'
elif [[ ${TYPE} == failed ]]; then
AUTHT=`cat rocketcauth.$$ | head -1`
USRHSH=`cat rocketcauth.$$ | tail -1`

curl -s -H "${TT}: ${AUTHT}"      -H "${XUI}: ${USRHSH}"      -H "${CS}:${CONT}"      ${RSERV}/api/v1/chat.postMessage      -d '{ "channel": "#general", "text": ":robot: ALERT: [FAILED JOBS]('$(echo ${NSERV})')! :fire:" }'
elif [[ ${TYPE} == lost ]]; then
AUTHT=`cat rocketcauth.$$ | head -1`
USRHSH=`cat rocketcauth.$$ | tail -1`

curl -s -H "${TT}: ${AUTHT}"      -H "${XUI}: ${USRHSH}"      -H "${CS}:${CONT}"      ${RSERV}/api/v1/chat.postMessage      -d '{ "channel": "#general", "text": ":robot: ALERT: [LOST JOBS]('$(echo ${NSERV})')! :fire:" }'
elif [[ ${TYPE} == running ]]; then
AUTHT=`cat rocketcauth.$$ | head -1`
USRHSH=`cat rocketcauth.$$ | tail -1`

curl -s -H "${TT}: ${AUTHT}"      -H "${XUI}: ${USRHSH}"      -H "${CS}:${CONT}"      ${RSERV}/api/v1/chat.postMessage      -d '{ "channel": "#general", "text": ":robot: ALERT: [NO RUNNING JOBS]('$(echo ${NSERV})')! :fire:" }'
elif [[ ${TYPE} == queued ]]; then
AUTHT=`cat rocketcauth.$$ | head -1`
USRHSH=`cat rocketcauth.$$ | tail -1`

curl -s -H "${TT}: ${AUTHT}"      -H "${XUI}: ${USRHSH}"      -H "${CS}:${CONT}"      ${RSERV}/api/v1/chat.postMessage      -d '{ "channel": "#general", "text": ":robot: ALERT: [QUEUED JOBS]('$(echo ${NSERV})')! :fire:" }'
else
